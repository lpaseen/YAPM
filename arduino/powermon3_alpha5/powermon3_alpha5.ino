 ...
 *
 * $Id: powermon3_alpha5.ino 1350 2016-03-14 18:43:46Z peters $
 *
 */
...
#define maxChannels 11 //how many channels that has something connected
...
unsigned int adcRead(byte channel){
  ...
  if (channel >= ADCmaxChannels-1){ //ADCmaxChannels:1 based, channel:0 based
    ...
    if (CurrentMUX!=MUX){ //need to move the mux
      ...
      delay(10); // give it time to settle, dunno how long is needed but 10ms should be more then enough.
    }
  }
...
double readrmsI(byte channel,double CT_RATIO){
  ...
  ch_valuesummary[channel]=(double)_sum/MAXsamples; //Raw ADC

#ifdef DEBUGI
  //  p("Channel %2d,filtered: %10ld, _sum: %-10ld, ch_valuesummary: %6u, _sum/MAXsamples: %10.3f _shifted_filter: %6ld \n",channel,_filtered_value,_sum,ch_valuesummary[channel],float(_sum/MAXsamples),_shifted_filter);
  /*
  p("Channel %2d,filtered: %10ld, _sum: %-10ld, ch_valuesummary: %6u, _sum/MAXsamples: %6u\n  sqrt(chval):%6u, sqrt2:%6u,\n  _shifted_filter: %6ld, lastVal:%6d \n",
    channel,_filtered_value,
    _sum,ch_valuesummary[channel],int(float(_sum/MAXsamples)),
    int(sqrt(ch_valuesummary[channel])),int(sqrt(float(_sum/MAXsamples))),
    _shifted_filter,_value);
  */

  p("Channel %2d,filtered: %10ld, _sum: %-10ld, ch_valuesummary: %6u, _sum/MAXsamples: %6u\n",
    channel,_filtered_value,_sum,ch_valuesummary[channel],int(float(_sum/MAXsamples)));
  ...
  Serial.print(_filtered_value);
  Serial.print(", _sum: ");
  Serial.print(_sum);
  Serial.print(", ch_valuesummary: ");
  Serial.print(ch_valuesummary[channel]);
  Serial.print(", _shifted_filter: ");
  Serial.print(_shifted_filter);
  Serial.println();
  */
#endif
...
//  ch_sumV[channel]=abs((double)_filtered_value*(double)ADC_step_mV/1000); // convert to milliV rms
...
// Set a initial value for the MUX
  ...
  for (i=0;i<5;i++){
    ...
    for (channel=1;channel<maxChannels;channel++){
      result=readrmsI(channel,2.5/120);
    }
    delay(500);
  }
...
*/
  ...
  for (channel=0;channel<maxChannels;channel++){
    ...
    case 0:
      ...
      ch_sumI[channel]=readVolt(channel,12.252); // Channel,RATIO+calibration (mainvolt/measurevolt, 120VAC->9VAC &  470k/47k=120/9/(470/47)=133.33+CAL+rms)
      ctimenow =  millis();
    ...
    case 1:
    case 2:
      // set in readrmsI //ctimethen =  millis();
      ...
      //      ch_sumI[channel]=readrmsI(channel,double(3.5355/111)); // Channel,CT_MAXVOLT/CT_MAXAMPS
      ch_sumI[channel]=readrmsI(channel,double(1.76777/111)); // Channel,CT_MAXVOLT/CT_MAXAMPS  (2.5/sqrt(2))/111
      // set in readrmsI //ctimenow =  millis();
//      buffvalues[buffhead].valuesummary[channel]=ch_sumI[channel];
    ...
    default:
      // set in readrmsI //ctimethen =  millis();
      ch_sumI[channel]=readrmsI(channel,double(1.0/30)); // Channel,CT_MAXVOLT/CT_MAXAMPS
      // set in readrmsI //ctimenow =  millis();
//      buffvalues[buffhead].valuesummary[channel]=ch_sumI[channel];
      buffvalues[buffhead].valuesummary[channel]=ch_valuesummary[channel];
      ch_samplingtime[channel]=ctimenow-ctimethen;
  ...
  totsamples++;
  if ( totsamples > 100 ) {buffhead++;} //let the filter settle before saving anything
